...
def save_to_csv(filename, sheetname, data):
    df = pd.DataFrame(data, columns=["Epoch", "Batch Time", "Forward NFE", "Backward NFE", "Train Accuracy",
                                     "Test Accuracy", "Train Loss", "Val Loss", "Test Loss"])
...
def get_epoch_loss(_model, _dataset_loader):
    _loss = 0
    for _x, _y in _dataset_loader:
        _x = _x.to(device)
        _y = _y.to(device)
        _logits = _model(_x)
        _loss += criterion(_logits, _y).item()

    return _loss / len(_dataset_loader)  # total _loss / number of batches
    ...
    for itr in range(args.nepochs * batches_per_epoch):
        ...
        if itr % batches_per_epoch == 0:
            with torch.no_grad():
                ...
                val_loss = get_epoch_loss(model, train_eval_loader)
                train_loss = get_epoch_loss(model, train_loader)
                test_loss = get_epoch_loss(model, test_loader)
                ...
                logger.info(
                    ...
                    "Train Acc {:.4f} | Test Acc {:.4f} | Train Loss {:.4f} | Val Loss {:.4f} | Test Loss {:.4f}".format(
                        itr // batches_per_epoch, batch_time_meter.val, batch_time_meter.avg, f_nfe_meter.avg,
                        b_nfe_meter.avg, train_acc, val_acc, train_loss, val_loss, test_loss
                    )
                )
                epoch_data.append([
